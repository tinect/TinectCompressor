...
//https://github.com/bluedaysolutions/jsqueeze
...
class JSqueeze
{
    ...
    public function squeeze($code, $singleLine = true, $keepImportantComments = true, $specialVarRx = false)
    {
        ...
        $this->keepImportantComments = (bool) $keepImportantComments;
        ...
        strpos($code, "\xC2\x85") !== false && $code = str_replace("\xC2\x85", "\n", $code); // Next Line
        strpos($code, "\xE2\x80\xA8") !== false && $code = str_replace("\xE2\x80\xA8", "\n", $code); // Line Separator
        strpos($code, "\xE2\x80\xA9") !== false && $code = str_replace("\xE2\x80\xA9", "\n", $code); // Paragraph Separator
        ...
        strpos($code, "\r") !== false && $code = strtr(trim($code), "\r", "\n");
    ...
    protected function extractStrings($f)
    {
            ...
            if ($instr) {
                if ($instr == '//') {
                    ...
                    if ($instr == '!') {
                        ...
                        if ($instr == "/'") {
                            while (isset($f[$i + 1]) && strpos('gmi', $f[$i + 1]) !== false) {
                                $s[] = $f[$i++];
                            }
                            $s[] = $f[$i];
                        }
                ...
                switch ($f[$i]) {
                    ...
                    case '/':
                        if ($f[$i + 1] == '*') {
                            ...
                            $a = $j && ($code[$j] == ' ' || $code[$j] == "\x7F") ? $code[$j - 1] : $code[$j];
                            if (strpos('-!%&;<=>~:^+|,()*?[{} ', $a) !== false
                                || (strpos('oenfd', $a) !== false
                                    && preg_match(
                    ...
                    case '+':
                    case '-':
                        if ($j && $f[$i] === $f[$i + 1] && $code[$j] == "\x7F" && $code[$j - 1] != ';') {
                            $code[$j] = ';';
                            $code[++$j] = "\x7F";
                            $code[++$j] = $f[$i];
                            $code[++$j] = $f[$i];
                    ...
                    case "\n":
                        if ($j > 3) {
                            if ($code[$j] == ' ' || $code[$j] == "\x7F") {
                                --$j;
                            }

                            $code[++$j] =
                                strpos('kend+-', $code[$j - 1]) !== false
                                && preg_match(
        ...
        strpos($code, 'new Array') !== false && $code = preg_replace("'new Array(?:\(\)|([;\])},:]))'", '[]$1', $code);
        strpos($code, 'new Object') !== false && $code = preg_replace("'new Object(?:\(\)|([;\])},:]))'", '{}$1', $code);
        ...
        if (preg_match_all("#([.,{]?(?:[gs]et )?)(?<![a-zA-Z0-9_\$])({$this->varRx})(:?)#", $closure, $w, PREG_SET_ORDER)) {
            foreach ($w as $k) {
                if (isset($k[1][0]) && ($k[1][0] === ',' || $k[1][0] === '{')) {
                    if ($k[3] === ':') {
                        $k = '.' . $k[2];
                    } elseif (substr($k[1], 1, 4) === 'get ' || substr($k[1], 1, 4) === 'set ') {
                ...
                for ($i = 0; $i < $a; ++$i) {
                    ...
                    if ($i % 2 === 1) {
                        if ($k[0] === ',' || $k[0] === '{') {
                            if (substr($k, -1) === ':') {
                                $k = '.' . substr($k, 1, -1);
                            } elseif (substr($k, 1, 4) === 'get ' || substr($k, 1, 4) === 'set ') {
    ...
    protected function renameVars(&$tree, $root)
    {
        if ($root) {
            ...
            foreach ($this->charFreq as $k => $v) {
                ...
                if (($k > 64 && $k < 91) || ($k > 96 && $k < 123)) { // A-Z a-z
                    $this->str0 .= $v;
                    $this->str1 .= $v;
                } elseif ($k > 47 && $k < 58) { // 0-9
                    $this->str1 .= $v;
                }
            }
                ...
                switch (substr($var, 0, 1)) {
                    case '.':
                        if (!isset($tree['local'][substr($var, 1)])) {
                            $tree['local'][$var] = '#' . ($this->specialVarRx && strlen($var) > 3 && preg_match("'^\.{$this->specialVarRx}$'", $var) ? $this->getNextName($tree) . '$' : substr($var, 1));
                        }
                        break;

                    case ';': $tree['local'][$var] = $root === 0 ? '' : $this->getNextName($tree);
                    // no break
                    case '#': break;

                    default:
                        $root = $this->specialVarRx && strlen($var) > 2 && preg_match("'^{$this->specialVarRx}$'", $var) ? $this->getNextName($tree) . '$' : $var;
                        $tree['local'][$var] = $root;
            ...
            foreach ($tree['local'] as $var => $root) {
                if ($tree['nfe'] !== $var) {
                    $tree['local'][$var] = $root === 0 ? '' : $this->getNextName($tree);
                }
            }
        }
    ...
    protected function getNewName($m)
    {
        ...
        if ($m[0] === ',' || $m[0] === '{' || $m[0] === ' ') {
            ...
            if (substr($m, -1) === ':') {
                $post = ':';
                $m = ($m[0] !== ' ' ? '.' : '') . substr($m, 1, -1);
            } elseif (substr($m, 1, 4) === 'get ' || substr($m, 1, 4) === 'set ') {
                $pre .= substr($m, 1, 4);
                $m = '.' . substr($m, 5);
            } else {
                $m = substr($m, 1);
